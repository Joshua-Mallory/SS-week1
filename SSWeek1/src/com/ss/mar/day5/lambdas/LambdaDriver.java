...
public class LambdaDriver {
	...
	public static void main(String[] args) {
		...
		List<String> myStrA = Arrays.asList("jote", "ape", "Davidian", "Art", "floundEr", "horse", "apple", "ask",
				"asp", "Ant");
		Setup(myStr, myInt, myStrA);
	}

	public static void Setup(List<String> myStr, List<Integer> myInt, List<String> myStrA) {
		...
		SortWUtils(myStr, predStringsEUtil);
		EvenOrOdd(myInt);
	...
	public static void SortWUtils(List<String> myStr, Predicate<String> pred) {
		List<String> sortedMyStrE = myStr.stream().filter(pred).collect(Collectors.toList());
		List<String> sortedMyStrNoE = myStr.stream().filter(pred).collect(Collectors.toList());
